...
const ItemUpdatePage: React.FC = () => {
  ...
  const [formValues, setFormValues] = useState<FormValues>({
    ...
    eventRate: product.discountRate.toString(),
    detailDescription: product.content,
    deliveryFee: '2500',
  ...
  useEffect(() => {
    const loadImages = async () => {
      if (product.imageUrl) {
        const mainImageFile = await fetchImageAsFile(product.imageUrl);
        setImagePreview(mainImageFile);
      }

      try {
        const response = await axios.get<string[]>(
          `http://localhost:8080/api/products-image/${product.productId}/SubImgURL`,
        );
        const imageFiles = await Promise.all(
          response.data.map(async (url) => await fetchImageAsFile(url)),
        );
